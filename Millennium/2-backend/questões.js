...
app.get("/dominio", (request, response) => {
  let db = new sqlite3.Database(DBPATH);
  let sql = "SELECT * FROM dominio WHERE idEixo = ?";
  db.all(sql, [], (err, rows) => {
...
app.get("/dominio", (request, response) => {
    let db = new sqlite3.Database(DBPATH);
    let sql = "SELECT * FROM questoes WHERE idEixo = ?";
    db.all(sql, [], (err, rows) => {
      response.statusCode = 200;
      response.json(rows);
    });
  ...
  app.post("/questoesCreate", (request, response) => {
    let db = new sqlite3.Database(DBPATH);
    let sql = "INSERT INTO Questao (texto, numeroQuestao, peso, idDominio, idAutor, versao) VALUES(?, ?, ?, ?, ?, ?)";
    console.log(request.body);
    let params = [];
    params.push(request.body.texto);
    params.push(request.body.numeroQuestao);
  ...
  app.post("/questoesDelete", (request, response) => {
    let db = new sqlite3.Database(DBPATH);
    let sql = "INSERT INTO Questao (texto, numeroQuestao, peso, idDominio, idAutor, versao) VALUES(?, ?, ?, ?, ?, ?)";
    console.log(request.body);
    let params = [];
