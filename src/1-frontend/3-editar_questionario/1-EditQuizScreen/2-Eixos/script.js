...
function createSectionCard(sectionId, sectionName, maxGrade) {
  ...
  <div>
  <i class="fa-regular fa-pen-to-square icon" onclick="setEixoAndRedirect(${sectionId},'${sectionName}',${maxGrade})"></i>
  </div>
  <div>
  <i class="fa-solid fa-trash-can icon" onclick="disableEixo(${sectionId}, '${sectionName}')"></i>
  </div>
  </div>
  </div>`;
  return cardElement;
