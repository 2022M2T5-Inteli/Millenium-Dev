...
function createescolaCard(questionarioId) {
  const newescolaCardHTML = `<div class="col-12">
  ...
  $("#escolaCardsSection").append(newescolaCardHTML);
}
...
async function loadSchoolQuestionariosCards() {
  ...
  $("#escolaCardsContainer").empty();
  questionarios.forEach((questionario) => {
    if (questionario.isComplete) {
      const escolaCard = createescolaCard(
        `Questionário #${questionario.id}`,
        questionario.id
      );
      $("#escolaCardsContainer").append(escolaCard);
    }
  });
  // escolaCardsContainer
}

function createescolaCard(name, id) {
